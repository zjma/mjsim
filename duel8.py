    ...
    def all_data():
        ...
        for my_wait_count in range(20):
            for my_hand_power in [1000, 2000, 4000, 8000, 12000, 16000]:
                data_points = [utils.simulate_duel(my_index=1, oppo_index=0, draw_pile_size=draw_pile_size, my_wait_count=my_wait_count, my_hand_power=my_hand_power, i_may_get_off=False, oppo_wait_count=6, oppo_hand_power=12000, oppo_may_get_off=False) for _ in range(10000)]
...
def plot():
    ...
    plt.xlabel('我方打点')
    plt.ylabel('我方听牌枚数')
    plt.title('两家对日期望得点(我方南家，敌方庄家，巡目13，我方全攻，敌方全攻，敌军打点12000听6枚，两家做牌独立)')
