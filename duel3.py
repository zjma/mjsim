...
def sample():
    def all_data():
        ...
        for my_wait_count in range(20):
            for my_hand_power in [1500, 3000, 6000, 12000, 18000, 24000]:
                data_points = [utils.simulate_duel(my_index=0, oppo_index=2, draw_pile_size=draw_pile_size, my_wait_count=my_wait_count, my_hand_power=my_hand_power, i_may_get_off=False, oppo_wait_count=6, oppo_hand_power=8000, oppo_may_get_off=False) for _ in range(10000)]
...
def plot():
    ...
    plt.xlabel('我方打点')
    plt.ylabel('我方听牌枚数')
    plt.title('两家对日期望得点(我方庄家，敌方西家，巡目7，我方全攻，敌方全攻，敌军打点8000听牌6枚，两家做牌独立)')
